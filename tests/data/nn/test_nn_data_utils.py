...
from replay.data.nn.utils import groupby_sequences, ensure_pandas
from replay.utils import get_spark_session, PandasDataFrame
...
def test_groupby_sequences_pandas(pandas_interactions, is_spark, sort_col):
    if is_spark is True:
        grouped = groupby_sequences(
            get_spark_session().createDataFrame(pandas_interactions),
            groupby_col="user_id",
            sort_col=sort_col
...
def test_ensure_pandas(pandas_interactions):
    assert isinstance(
        ensure_pandas(get_spark_session().createDataFrame(pandas_interactions)),
        PandasDataFrame
    )
