...
from replay.utils import spark_utils
...
def test_process_timestamp(log_data, ground_truth_data, schema, spark):
    ...
    for col in log.columns:
        ...
        log = spark_utils.process_timestamp_column(log, col, **kwargs)
...
def test_func_get():
    vector = np.arange(2)
    assert spark_utils.func_get(vector, 0) == 0.0
...
def test_convert():
    ...
    spark_df = spark_utils.convert2spark(dataframe)
    pd.testing.assert_frame_equal(dataframe, spark_df.toPandas())
    assert spark_utils.convert2spark(spark_df) is spark_df


def test_sample_top_k(long_log_with_features):
    res = spark_utils.sample_top_k_recs(long_log_with_features, 1, seed=123)
    assert (
        res.count()
...
def test_get_unique_entities(spark, array):
    ...
    assert sorted(
        list(spark_utils.get_unique_entities(array or log, "test").toPandas()["test"])
    ) == [1, 2, 3]
