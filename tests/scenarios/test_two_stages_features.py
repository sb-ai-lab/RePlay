...
def log_second_stage(spark):
    ...
    return spark.createDataFrame(
        data=[
            ...
            [1, 0, date, 1.0],
            [1, 2, datetime(2018, 1, 1), 4.0],
            [2, 2, date, 3.0],
            [2, 1, date, 2.0],
            [2, 3, datetime(2020, 3, 1), 1.0],
        ],
...
def item_features_second_stage(spark):
    return spark.createDataFrame(
        [(0, 4.0, "cat"), (1, 10.0, "dog"), (3, 0.0, "cat")]
...
def test_second_level_fp_log_features_unary_relevance_no_timestamp(
    ...
    gt_user_features = spark.createDataFrame(
        data=[
            [0, math.log(2), (math.log(2) + math.log(2)) / 2],
            [1, math.log(2), (math.log(2) + math.log(2)) / 2],
            [2, math.log(3), (math.log(2) + math.log(2) + math.log(1)) / 3],
        ],
    ...
    gt_item_features = spark.createDataFrame(
        data=[
            ...
            [2, math.log(2), (math.log(3) + math.log(2)) / 2],
            [3, math.log(2), (math.log(3) + math.log(2)) / 2],
        ],
...
def test_second_level_fp_log_features_unary_relevance_timestamp(
    ...
    gt_user_time_features = spark.createDataFrame(
        data=[
            [0, date, datetime(2019, 1, 5), math.log(2)],
            [1, datetime(2018, 1, 1), date, math.log(2)],
            [2, date, datetime(2020, 3, 1), math.log(2)],
        ],
...
def test_second_level_fp_log_features_relevance(
    ...
    gt_user_abnorm_features = spark.createDataFrame(
        data=[
            [0, (abs(1.0 - 1.0) + abs(2.0 - 3.0)) / 2, 0.0],
            [1, (abs(1.0 - 1.0) + abs(4.0 - 3.5)) / 2, 0.03125],
            [
                2,
