...
from tests.utils import spark, long_log_with_features, short_log_with_features, user_features, sparkDataFrameEqual
...
def test_works(long_log_with_features,short_log_with_features,users_features):
    ...
    res = model.optimize(long_log_with_features, short_log_with_features, users_features, k=1, budget=1)
...
def test_cold_user(long_log_with_features, users_features):
    ...
    res = model.predict(users_features, 2, users=convert2spark(pd.DataFrame({"user_idx" : [1]})))
    assert res.count() == 2
