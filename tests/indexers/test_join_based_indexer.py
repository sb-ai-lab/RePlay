...
def test_join_based_indexer(spark, log):
    ...
    actual_unique_user_ids = (
        df_with_primary_indexes.select("user_id").distinct().sort("user_id").collect()
    )
    ...
    actual_unique_item_ids = (
        df_with_primary_indexes.select("item_id").distinct().sort("item_id").collect()
    )
    ...
    new_log_part = spark.createDataFrame(
            [(100, 1), (100, 2)]
    ).toDF("user_id", "item_id")
    ...
    assert indexed_df.join(new_indexed_df, on=["user_idx", "item_idx"]).distinct().count() == indexed_df.distinct().count()
    ...
    assert indexed_new_log_part_df.select("user_idx").distinct().count() == new_log_part.select(
        "user_id").distinct().count()
    assert indexed_new_log_part_df.select("item_idx").distinct().count() == new_log_part.select(
