...
def user_features(spark):
    ...
    ).toDF("user_idx", "user_feature_cat", "user_feature_cont")


@pytest.fixture
def item_features(spark):
    return spark.createDataFrame(
        [(0, 4.0, 5.0), (1, 5.0, 4.0), (2, 1.5, 3.3)]
    ).toDF("item_idx", "item_feature_cat", "item_feature_cont")
...
def model_with_features():
    cols_item = {'continuous_cols':["item_feature_cont"],
                 'cat_embed_cols':["item_feature_cat"],
                 'wide_cols': ["item_feature_cat", "item_feature_cont"]}

    cols_user = {'continuous_cols':["user_feature_cont"],
...
@pytest.mark.experimental
def test_use_features(model_with_features,
                      user_features,
                      item_features,
                      log):
    model_with_features.fit(log,
                            user_features=user_features,
                            item_features=item_features)
    ...
    base_pred = model_with_features._predict(log,
                                             5,
                                             users=users,
                                             items=items,
                                             user_features=user_features,
