...
class Array2DColumn:
    def __init__(
        ...
        self.inner_offsets: torch.LongTensor = get_offsets(inner_lengths)
        self.outer_offsets: torch.LongTensor = get_offsets(outer_lengths)
        if len(shape) != 2:
...
def to_torch(
    ...
    flatten_dim0: pa.Array = pc.list_flatten(array)
    flatten: pa.Array = pc.list_flatten(flatten_dim0)

    outer_lengths: pa.Array = pc.list_value_length(array).cast(pa.int64())
    inner_lengths: pa.Array = pc.list_value_length(flatten_dim0).cast(pa.int64())
