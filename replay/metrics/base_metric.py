...
def preprocess_gt(
    ...
    true_items_by_users = ground_truth.groupby(query_column).agg(
        sf.collect_set(item_column).alias("ground_truth")
    )
    if ground_truth_users is not None:
        true_items_by_users = true_items_by_users.join(
            ground_truth_users, on=query_column, how="right"
        )
        true_items_by_users = fill_na_with_empty_array(
            true_items_by_users,
            "ground_truth",
            ground_truth.schema[item_column].dataType,
        )
...
def drop_duplicates(
    ...
    return (
        recommendations.withColumn(
            "_num",
            sf.row_number().over(
                Window.partitionBy(query_column, item_column).orderBy(sf.col(rating_column).desc())
            ),
        )
        .where(sf.col("_num") == 1)
        .drop("_num")
...
def filter_sort(
    ...
    """
    item_type = recommendations.schema[item_column].dataType
    ...
    recommendations = (
        ...
        .agg(
            sf.collect_list(
                sf.struct(*[c for c in [rating_column, item_column, extra_column] if c is not None]))
            .alias("pred_list"))
    ...
    selection = [
        query_column,
        sf.col(f"pred_list.{item_column}")
...
def get_enriched_recommendations(
    ...
    recommendations = get_top_k_recs(recommendations, k=max_k, query_column=query_column, rating_column=rating_column)
    ...
    joined = filter_sort(
        ...
        true_items_by_users, how="right", on=[query_column]
    )

    return fill_na_with_empty_array(
        joined, "pred", recommendations.schema[item_column].dataType
    )
...
class Metric(ABC):
    ...
    def _get_metric_distribution(self, recs: DataFrame, k: int) -> DataFrame:
        ...
        distribution = recs.rdd.flatMap(
            ...
            f"{self.query_column} {recs.schema[self.query_column].dataType.typeName()}, value double"
        )
        return distribution
    ...
    def user_distribution(
        ...
        count = log.groupBy(self.query_column).count()
        ...
        for cut_off in k_list:
            ...
            val = count.join(dist, on=self.query_column, how="right").fillna(
                0, subset="count"
            )
            val = (
                val.groupBy("count")
...
class NCISMetric(Metric):
    ...
    def __init__(
        ...
        self.prev_policy_weights = convert2spark(
            prev_policy_weights
        ).withColumnRenamed(self.rating_column, "prev_relevance")
    ...
    def _softmax_by_user(df: DataFrame, col_name: str, query_column: str = "user_idx") -> DataFrame:
        ...
        return (
            df.withColumn(
                "_min_rel_user",
                sf.min(col_name).over(Window.partitionBy(query_column)),
            )
            .withColumn(
            ...
            .withColumn(
                col_name,
                sf.col(col_name)
                / sf.sum(col_name).over(Window.partitionBy(query_column)),
            )
            .drop("_min_rel_user")
        )
    ...
    def _weigh_and_clip(
        ...
        return (
            ...
            .select(query_column, item_column, rating_column, "weight")
        )

    def _reweighing(self, recommendations):
        if self.activation == "softmax":
            recommendations = self._softmax_by_user(
                recommendations, col_name="prev_relevance", query_column=self.query_column,
            )
            recommendations = self._softmax_by_user(
                recommendations, col_name=self.rating_column, query_column=self.query_column,
            )
            ...
            recommendations = self._sigmoid(
                recommendations, col_name=self.rating_column
            )

        return self._weigh_and_clip(
            df=recommendations,
            threshold=self.threshold,
            target_policy_col=self.rating_column,
            query_column=self.query_column,
            item_column=self.item_column,
            rating_column=self.rating_column
        )
    ...
    def _get_enriched_recommendations(
        ...
        recommendations = recommendations.join(
            true_items_by_users, how="right", on=[self.query_column]
        )
        return fill_na_with_empty_array(
