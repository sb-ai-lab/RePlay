...
class BasePredictionCallback(lightning.Callback, Generic[_T]):
    ...
    def on_predict_batch_end(
        ...
        scores = self._compute_pipeline(batch, outputs)
    ...
    def _compute_pipeline(self, batch: PredictionBatch, scores: torch.Tensor) -> list[torch.LongTensor, torch.Tensor]:
        for postprocessor in self._postprocessors:
            scores = postprocessor.on_prediction(batch, scores)
        return scores
...
class TorchPredictionCallback(BasePredictionCallback[list[torch.LongTensor, torch.LongTensor, torch.Tensor]]):
    """
    ...
    def _ids_to_result(
        ...
        item_ids: torch.Tensor,
        item_scores: torch.Tensor,
    ) -> list[torch.LongTensor, torch.LongTensor, torch.Tensor]:
        return (
