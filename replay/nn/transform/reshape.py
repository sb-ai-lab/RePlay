...
class UnsqueezeTransform(torch.nn.Module):
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        if self.dim > batch[self.feature_name].ndim - 1:
            msg = (
                "The dim parameter is incorrect."
                f"Expected unsqueezing by {self.dim} dimension,"
                f"but got the tensor with {batch[self.feature_name].ndim} dimensions."
            )
            raise ValueError(msg)

        output_batch = {k: v for k, v in batch.items() if k != self.feature_name}
        output_batch[self.feature_name] = batch[self.feature_name].unsqueeze(self.dim)
