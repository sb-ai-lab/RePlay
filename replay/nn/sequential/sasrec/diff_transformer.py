...
class MultiHeadDifferentialAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        assert (
            kdim % num_heads == 0
        ), "Query/Key embedding dim is not divisible by num_heads"
        assert (
            vdim % num_heads == 0
        ), "Value embedding dim is not divisible by num_heads"
        ...
        self.W_q = torch.nn.Linear(
            embedding_dim, 2 * self.qk_e_head * num_heads, bias=bias
        )
        ...
        self.register_buffer(
            "scaling", torch.asarray(1 / math.sqrt(self.qk_e_head), dtype=torch.float32)
        )
    ...
    def forward(
        ...
        query = self.W_q(
            query
        )  # Shape: (batch_size, seq_len, 2 * num_heads * qk_e_head)
        ...
        query = query.view(
            batch_size, seq_len, self.num_heads, 2 * self.qk_e_head
        ).transpose(1, 2)
        key = key.view(
            batch_size, seq_len, self.num_heads, 2 * self.qk_e_head
        ).transpose(1, 2)
        value = value.view(
            batch_size, seq_len, self.num_heads, self.v_e_head
        ).transpose(1, 2)

        # Split query and key into query1, query2 and key1, key2
        query1, query2 = query.chunk(
            2, dim=-1
        )  # Each of shape: (batch_size, num_heads, seq_len, d_head)
        key1, key2 = key.chunk(
            2, dim=-1
        )  # Each of shape: (batch_size, num_heads, seq_len, d_head)
        ...
        if len(attn_mask.shape) == 3:
            attn_mask = attn_mask.reshape(
                attn_mask.shape[0] // self.num_heads,
                self.num_heads,
                *attn_mask.shape[1:]
            )
        ...
        attention_scores1 = torch.matmul(
            query1, key1.transpose(-2, -1)
        ) * self.get_buffer(
            "scaling"
        ...
        attention1 = torch.nn.functional.softmax(
            attention_scores1, dim=-1
        )  # (batch_size, num_heads, seq_len, seq_len)
        ...
        output_normalized = (
            output_reshaped / rms_norm
        ) * self.rms_scale  # (batch*num_heads, seq_len, v_e_head)

        # Reshape back to (batch_size, num_heads, seq_len, v_e_head)
        output_normalized = output_normalized.view(
            batch_size, self.num_heads, seq_len, self.v_e_head
        )
        ...
        output_concat = (
            output_normalized.transpose(1, 2)
            .contiguous()
            .view(batch_size, seq_len, self.num_heads * self.v_e_head)
        )

        # Final linear projection
        output_projection = self.W_o(
            output_concat
...
class DiffTransformerBlock(torch.nn.Module):
    ...
    def __init__(self, embedding_dim: int, num_heads: int, lambda_init: float):
        ...
        self.attn = MultiHeadDifferentialAttention(
            embedding_dim, num_heads, lambda_init, vdim=2 * embedding_dim
        )
