...
class TokenMaskTransform(BaseTransform):
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        assert (
            paddings.dtype == torch.bool
        ), "Source tensor for token mask should be boolean."

        mask_prob = torch.rand(
            paddings.size(-1), dtype=torch.float32, generator=self.generator
        ).to(device=paddings.device)
