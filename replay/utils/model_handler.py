...
if PYSPARK_AVAILABLE:
    ...
    from .spark_utils import convert2spark, load_pickled_from_parquet, save_picklable_to_parquet
...
def save(
    ...
    if is_many_frameworks_model and model.is_fitted:
        ...
        client_params_path = join(path, "client_params.json")
        df = spark.read.json(sc.parallelize([json.dumps(client_params)]))
        df.coalesce(1).write.mode("overwrite").option("ignoreNullFields", "false").json(client_params_path)
        model = model._impl
    ...
    for name, df in dataframes.items():
        if df is not None:
            convert2spark(df).write.mode("overwrite").parquet(join(df_path, name))

    if hasattr(model, "fit_queries"):
        convert2spark(model.fit_queries).write.mode("overwrite").parquet(join(df_path, "fit_queries"))
    if hasattr(model, "fit_items"):
        convert2spark(model.fit_items).write.mode("overwrite").parquet(join(df_path, "fit_items"))
...
def load(path: str, model_type=None) -> _BaseRecommenderSparkImpl:
    ...
    if name in [i.__name__ for i in implementations_list]:
        ...
        realization = (
            "spark"
            if "Spark" in str(impl_class)
            else "pandas" if "Pandas" in str(impl_class) else "polars" if "Polars" in str(impl_class) else None
        )
