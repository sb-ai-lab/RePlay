    ...
    if path.startswith("hdfs://"):
        if path.startswith("hdfs:///"):
            ...
            if default_fs.startswith("hdfs://"):
                ...
                raise Exception(f"Can't get default hdfs uri for path = '{path}'. Set direct like 'hdfs://host:port/dir/file' or configure hadoop.")
        else:
...
# def build_hnsw_index(item_vectors: DataFrame, features_col: str, params: Dict[str, Any]):
...
#                     fs.copy_files("file://" + tmp_file_path, hdfs_uri + index_path)
...
#         build_index = create_udf_func(params["method"], params["space"], to_hdfs, default_fs, params["index_path"])
...
# def filter_seen_hnsw_res(log: DataFrame, pred: DataFrame, k: int, id_type="idx"):
#     """
...
# def infer_hnsw_index(log: DataFrame, user_vectors: DataFrame, features_col: str, params: Dict[str, Any], k: int, max_items_to_retrieve: int):
...
#     k_udf = k + max_items_to_retrieve

#     @pandas_udf(
#         "item_idx array<int>, distance array<double>, user_idx int"
#     )
#     def infer_index(
...
#             temp_path = tempfile.mkdtemp()
#             tmp_file_path = os.path.join(temp_path, "nmslib_hnsw_index")                    
#             fs.copy_files(default_fs + params["index_path"], "file://" + tmp_file_path)
...
#     ):
#         res = filter_seen_hnsw_res(log, res, k)
#         res = res.cache()
#         # res.write.mode("overwrite").format("noop").save()
