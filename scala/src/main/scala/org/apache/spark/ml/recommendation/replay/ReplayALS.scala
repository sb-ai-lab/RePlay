...
private[recommendation] trait ALSModelParams extends Params with HasPredictionCol
    ...
    n match {
      ...
      case v: Number =>
        ...
        if (v.doubleValue == intV) {
          ...
          throw new IllegalArgumentException(s"ReplayALS only supports values in Integer range " +
      ...
      case _ => throw new IllegalArgumentException(s"ReplayALS only supports values in Integer range " +
  ...
  val coldStartStrategy = new Param[String](this, "coldStartStrategy",
    ...
    s"${ReplayALSModel.supportedColdStartStrategies.mkString(",")}.",
    (s: String) =>
      ReplayALSModel.supportedColdStartStrategies.contains(s.toLowerCase(Locale.ROOT)))
...
private[recommendation] trait ALSParams extends ALSModelParams with HasMaxIter with HasRegParam
  ...
  /**
   * Param for StorageLevel for ReplayALS model factors. Pass in a string representation of
   * `StorageLevel`.
   * Default: "MEMORY_AND_DISK".
   *
   * @group expertParam
  ...
  val finalStorageLevel = new Param[String](this, "finalStorageLevel",
    "StorageLevel for ReplayALS model factors.",
 ...
 */
@Since("1.3.0")
class ReplayALSModel private[ml] (
    @Since("1.4.0") override val uid: String,
    @Since("1.4.0") val rank: Int,
    @transient val userFactors: DataFrame,
    @transient val itemFactors: DataFrame)
  extends Model[ReplayALSModel] with ALSModelParams with MLWritable {
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    getColdStartStrategy match {
      case ReplayALSModel.Drop =>
        predictions.na.drop("all", Seq($(predictionCol)))
      case ReplayALSModel.NaN =>
        predictions
    }
  }
  ...
  @Since("1.5.0")
  override def copy(extra: ParamMap): ReplayALSModel = {
    val copied = new ReplayALSModel(uid, rank, userFactors, itemFactors)
    copyValues(copied, extra).setParent(parent)
  }

  @Since("1.6.0")
  override def write: MLWriter = new ReplayALSModel.ALSModelWriter(this)

  @Since("3.0.0")
  override def toString: String = {
    s"ReplayALSModel: uid=$uid, rank=$rank"
  }
...
@Since("1.6.0")
object ReplayALSModel extends MLReadable[ReplayALSModel] {
  ...
  @Since("1.6.0")
  override def read: MLReader[ReplayALSModel] = new ALSModelReader

  @Since("1.6.0")
  override def load(path: String): ReplayALSModel = super.load(path)

  private[ReplayALSModel] class ALSModelWriter(instance: ReplayALSModel) extends MLWriter {
  ...
  private class ALSModelReader extends MLReader[ReplayALSModel] {

    /** Checked against metadata when loading model */
    private val className = classOf[ReplayALSModel].getName

    override def load(path: String): ReplayALSModel = {
      ...
      val model = new ReplayALSModel(metadata.uid, rank, userFactors, itemFactors)
 ...
 */
@Since("1.3.0")
class ReplayALS(@Since("1.4.0") override val uid: String) extends Estimator[ReplayALSModel] with ALSParams
  with DefaultParamsWritable {

  import org.apache.spark.ml.recommendation.replay.ReplayALS.Rating
  ...
  @Since("2.0.0")
  override def fit(dataset: Dataset[_]): ReplayALSModel = instrumented { instr =>
    transformSchema(dataset.schema)
    ...
    val (userFactors, itemFactors) = ReplayALS.train(ratings, rank = $(rank),
    ...
    val model = new ReplayALSModel(uid, $(rank), userDF, itemDF).setBlockSize($(blockSize))
      .setParent(this)
    copyValues(model)
  }
  ...
  @Since("1.5.0")
  override def copy(extra: ParamMap): ReplayALS = defaultCopy(extra)
}
 ...
 * than 2 billion.
 */
object ReplayALS extends DefaultParamsReadable[ReplayALS] with Logging {
  ...
  @Since("1.6.0")
  override def load(path: String): ReplayALS = super.load(path)
  ...
  def train[ID: ClassTag]( // scalastyle:ignore
    ...
    require(intermediateRDDStorageLevel != StorageLevel.NONE,
      "ReplayALS is not designed to run without persisting intermediate RDDs.")
   ...
   *
   * The ReplayALS algorithm partitions the columns of the users factor matrix evenly among Spark workers.
   * Since each column of the factor matrix is calculated using the known ratings of the correspond-
   * ing user, and since the ratings don't change across iterations, the ReplayALS algorithm preshuffles
   ...
   * Caching of the input factors is handled in [[ReplayALS#train]].
   */
  ...
  /**
   * Partitioner used by ReplayALS. We require that getPartition is a projection. That is, for any key k,
