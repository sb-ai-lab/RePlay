...
def eval_baselines(dataset, bandit_feedback_train, bandit_feedback_test):
    ...
    action_dist_nn_dm,\
    action_dist_nn_ipw,\
    action_dist_nn_dr = [m.predict_proba(context=bandit_feedback_test["context"]) for m in nn_methods]

    clss = [LogisticRegression(C=100, random_state=12345),
            RandomForest(n_estimators=30, min_samples_leaf=10, random_state=12345)]

    ipw_methods = [IPWLearner(n_actions=dataset.n_actions,
                              base_classifier=c,
                              len_list=dataset.len_list) for c in clss]

    for m in ipw_methods:
        m.fit(context=bandit_feedback_train["context"],
              action=bandit_feedback_train["action"],
              reward=bandit_feedback_train["reward"],
              pscore=bandit_feedback_train["pscore"],
              position=bandit_feedback_train["position"])
