...
def get_dataset(ds_name: str) -> tuple[pd.DataFrame, dict[str, str]]:
    ...
    col_mapping = {"user_id": "user_id", "item_id": "item_id", "relevance": "rating", "timestamp": "timestamp"}
...
def main():
    ...
    parser.add_argument("--scale", dest="action_randomization_scale", type=float, required=False, default=0.01)
    ...
    only_negatives_log = log.filter(sf.col("relevance") < 3).withColumn("relevance", sf.lit(0.0))
    ...
    pos_neg_train = train.withColumn("relevance", sf.lit(1.0)).union(
        indexer.transform(only_negatives_log.filter(sf.col("timestamp") < test_start))
    )
    pos_neg_train.cache()
    pos_neg_train.count()

    experiment = Experiment(
        test, {MAP(): K, NDCG(): K, HitRate(): K_list_metrics, Coverage(train): K, Surprisal(train): K, MRR(): K}
    )
    ...
    for name in tqdm.tqdm(algorithms.keys(), desc="Model"):
        ...
        if name == f"CQL_{max(n_epochs)}":
            ...
            # noinspection PyTypeChecker
            fit_predict_add_res(
                name + "_valid", model, experiment, train=train_, top_k=K, test_users=test_users, predict_only=True
            )

        print(
            experiment.results[[f"NDCG@{K}", f"MRR@{K}", f"Coverage@{K}", "fit_time"]].sort_values(
                f"NDCG@{K}", ascending=False
            )
        )
