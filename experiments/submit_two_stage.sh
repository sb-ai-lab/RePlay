...
SEED=${SEED:-"42"}
K=${K:-"100"}
K_LIST_METRICS=${K_LIST_METRICS:-"5,10,50,100"}
USE_BUCKETING=${USE_BUCKETING:-"True"}
ALS_RANK=${ALS_RANK:-"100"}
NUM_NEIGHBOURS=${NUM_NEIGHBOURS:-"100"}
...
CHECK_NUMBER_OF_ALLOCATED_EXECUTORS=${CHECK_NUMBER_OF_ALLOCATED_EXECUTORS:-"False"} # True

EXECUTOR_INSTANCES=${EXECUTOR_INSTANCES:-"2"}
DATASET=${DATASET:-"MovieLens__1m"} # MillionSongDataset MovieLens__25m Netflix
MODEL=${MODEL:-"two_stage"}
...
--conf 'spark.executor.extraClassPath=/jars/spark_3.1.1_synapseml_0.9.5_jars/*' \
--conf 'spark.driver.extraClassPath=/jars/spark_3.1.1_synapseml_0.9.5_jars/*' \
...
--py-files 'dist/replay_rec-0.10.0-py3-none-any.whl,experiments/experiment_utils.py,/SLAMA/dist/SparkLightAutoML_DEV-0.3.2-py3-none-any.whl' \
--files '/SLAMA/tabular_config.yml' \
--num-executors ${EXECUTOR_INSTANCES} \
--jars 'scala/target/scala-2.12/replay_2.12-0.1.jar,/SLAMA/jars/spark-lightautoml_2.12-0.1.1.jar' \
--name ${EXPERIMENT} \
"${SCRIPT}"

# launch example:
# poetry build && ./experiments/submit_two_stage.sh experiments/run_two_stage.py

#--conf 'spark.yarn.am.waitTime=1500000' \