...
from pyspark.sql.functions import col, count, row_number, sum, lit, sqrt
...
class KNNRecommender(BaseRecommender):
    ...
    def _get_similarity_matrix(
        ...
        return (
            ...
            .join(
                ...
                how="inner",
                on=col("item_id_one") > col("item_id_two")
            )
            .join(
                dot_products,
                how="inner",
            ...
            .join(
                ...
                how="inner",
                on=col("item1.item_id") == col("item_id_one")
            )
            .join(
                item_norms.alias("item2"),
                how="inner",
                on=col("item2.item_id") == col("item_id_two")
            )
            .withColumn(
                "similarity",
                col("dot_product") /
                (col("item1.norm") * col("item2.norm") + self.shrink)
            )
    ...
    def _get_k_most_similar(self, similarity_matrix: DataFrame) -> DataFrame:
        ...
        return (
            ...
            .union(
                similarity_matrix
                .select(
                    col("item_id_two").alias("item_id_one"),
                    col("item_id_one").alias("item_id_two"),
                    col("similarity")
                )
            )
            .withColumn(
            ...
            .filter(col("similarity_order") <= self.k)
    ...
    def _fit(
        ...
        dot_products = (
            ...
            .agg(count("user_id").alias("dot_product"))
            .cache()
        )
        item_norms = (
            log
            .groupby("item_id")
            .agg(count("user_id").alias("square_norm"))
            .select(col("item_id"), sqrt("square_norm").alias("norm"))
            .cache()
        )
    ...
    def _predict(
        ...
        recs = (
            ...
            .join(
                self.similarity,
                how="left",
                on=col("item_id") == col("item_id_one")
            )
            .groupby("user_id", "item_id_two")
            .agg(sum("similarity").alias("relevance"))
        ...
        recs = recs.filter(col("relevance") > 0.0)
        return recs.cache()
