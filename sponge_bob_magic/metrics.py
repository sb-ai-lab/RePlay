...
class NDCG(Metric):
    ...
    чем ближе к началу списка полезные рекомендации, тем больше её значение.
...
class MAP(Metric):
    """
    Mean Average Precision -- усреднение ``Precision`` по целомым числам от 1 до ``K``, усреднённое по пользователям.

    .. math::
        &AP@K(i) = \\frac 1K \sum_{j=1}^{K}\mathbb{1}_{r_{ij}}Precision@j(i)

        &MAP@K(i) = \\frac 1K \sum_{j=1}^{K}AP@k(i)
...
class Recall(Metric):
    ...
    Усредняется по всем пользователям.
